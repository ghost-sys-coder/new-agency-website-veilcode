...
/* ─── VISUAL PANEL ────────────────────────────────────────────────────────── */
/* Shows the first screenshot if src is set, otherwise the animated rings.    */

function CardVisual({ project: p }: { project: Project }) {
    const { classes: c } = p;
    ...
    return (
        ...
        >
            {hasImage ? (
                <>
                    {/* Real screenshot — fills the panel, zooms subtly on hover */}
                    <Image
                        src={heroShot!.src!}
                        alt={heroShot!.alt}
                        fill
                    ...
                    />
                    {/* Gradient scrim so the service badge stays legible */}
                    <div
                        aria-hidden
                        className="absolute inset-0
                            bg-linear-to-t from-surface/60 via-transparent to-transparent"
                    />
                </>
            ) : (
                <>
                    {/* Fallback — grid texture + animated rings */}
                    <div
                        aria-hidden
            ...
            {/* Year — always visible */}
            <span className="absolute bottom-3 right-3 z-10 font-mono text-[10px]
